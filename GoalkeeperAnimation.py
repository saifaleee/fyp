...
class GoalkeeperAnimator:
    def __init__(self, animations_folder="Fbx Animations"):
        ...
        if not os.path.exists(animations_folder):
            ...
            print(f"Full path: {os.path.abspath(animations_folder)}")
        else:
            print(f"Found animations folder: {os.path.abspath(animations_folder)}")
            # List contents of the folder
            print(f"Contents of {animations_folder}:")
            for item in os.listdir(animations_folder):
                item_path = os.path.join(animations_folder, item)
                if os.path.isdir(item_path):
                    print(f"  - {item}/ (directory with {len(os.listdir(item_path))} files)")
                else:
                    print(f"  - {item} (file)")
    ...
    def _load_animation(self, folder_path):
        ...
        for file in sorted(os.listdir(folder_path)):
            ...
            if file.lower().endswith('.png'):
                ...
                if img is not None:
                    print(f"    Successfully loaded image: {file} (shape: {img.shape})")
                    frames.append(img)
                else:
        ...
        if not frames:
            ...
            if png_files:
                print(f"  However, found these potential PNG files: {png_files}")
                print(f"  Check file extensions - they should be lowercase .png")
        else:
            print(f"Loaded {len(frames)} animation frames from {folder_path}")
    ...
    def set_animation(self, direction, total_video_frames=None, animation_speed=0.5, y_offset_percentage=0.25):
        ...
        if total_video_frames is not None:
            ...
            print(f"Animation will play at {animation_speed*100}% speed over {total_video_frames} video frames")
            print(f"Animation will be positioned {y_offset_percentage*100}% lower")
        
        print(f"Set animation to '{direction}' ({len(self.current_animation)} frames)")
